"""
Fanqie Novel font-based character decryption.

Fanqie uses Unicode Private Use Area characters (U+E3D8 to U+E54B) as a substitution
cipher. Each codepoint in that range maps to a real Chinese character or ASCII character.
This mapping is static (not per-request), but may change over time.
"""

from __future__ import annotations

# Character mapping table: index -> real character
# Source: reverse-engineered from fanqienovel.com's rendering
# Range: U+E3D8 (58344) to U+E54B (58715), 372 entries
_CODE_START = 58344  # 0xE3D8
_CODE_END = 58715    # 0xE54B
...
def decrypt_char(char: str) -> str:
    """Decrypt a single character. Returns the original if not in the cipher range."""
    code = ord(char)
    if _CODE_START <= code <= _CODE_END:
        index = code - _CODE_START
...
def decrypt_text(text: str) -> str:
    """Decrypt a full string of Fanqie-encoded text."""
    return "".join(decrypt_char(c) for c in text)


def is_encrypted(text: str) -> bool:
    """Check if text contains Fanqie-encrypted characters."""
    for char in text:
        code = ord(char)
        if _CODE_START <= code <= _CODE_END:
            return True
